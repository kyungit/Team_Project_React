    ...
    return(
        ...
        {imageInfo && imageInfo.map((reservation , index) => (
            <Box key={index} className='mb-24 p-12'>
                <Row className="text-2xl m-0 font-bold text-black">{`${index + 1}. ${reservation.d_name} ${reservation.d_type}`}</Row>
                <Row className="w-3/5 h-80">
                    <img src={reservation.r_img} alt=""/>
                </Row>
                    {/*<Row className="mt-1">{`이름이야 : ${reservation.r_name}`}</Row>*/}
                    <Row className="text-2xl mt-1">{`예약자아이디 : ${reservation.m_userid}`}</Row>
                    ...
                    <Row className="text-2xl mt-1">{`인원수 : ${reservation.reservation_guest}명`}</Row>
