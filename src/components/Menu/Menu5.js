...
export default function Menu5() {
    ...
    return (
        <div className="col-start-4 col-end-13 w-full h-auto pt-16">
            { imageInfo && imageInfo.map((review,index)=> (
                ...
                <Box key={index}>
                    ...
                    {review.fileInfo  && review.fileInfo.length > 0 &&
                        ...
                        {review.fileInfo.map((e, i) => (
                            <div key={i}>
                                <img src={`http://localhost:8080/file/${e.review_stored}`} alt="미리보기" style={{
                                    width: '100%',
                                    height: '100%',
                    ...
                    {/* 리뷰 작성 모달창입니다. */}
                    {isModalOpen && (
                        <ReviewModifyModal closeModal={closeModal}>
